...
function attachEnhancement(root: HTMLElement, signal: AbortSignal): void {
	...
	const popover = (
		<div id={popoverId} className="rgh-pr-diffstat-by-ext-popover" />
	...
	const positionPopover = (): void => {
		...
		popover.style.left = `${Math.min(Math.max(rect.right - width, margin), window.innerWidth - width - margin)}px`;
		popover.style.top = `${Math.min(rect.bottom + margin, window.innerHeight - margin)}px`;
	};
	...
	observe(
		...
		element => {
			if (element instanceof HTMLElement) {
				attachEnhancement(element, signal);
			}
		},
		{signal},
	);
